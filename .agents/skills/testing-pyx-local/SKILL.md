...
# Evidence
- Campaign reservations need adversarial runtime coverage, not only happy-path log sends. Import a fresh QA batch through UI, then add one phone to opt_outs after import. With daily_limit=1, assert late opt-out clears sent_at and releases last_sent_at so the next contact is not blocked by quota/interval.
- To exercise a provider failure without delivering messages, temporarily restart with WHATSAPP_PROVIDER=evolution and EVOLUTION_API_URL, EVOLUTION_API_KEY, EVOLUTION_INSTANCE explicitly empty. Assert falhou, a configuration failure reason, null sent_at, released campaign timestamp and zero outbound. Pause the fixture promptly to avoid the internal timer consuming its next contact; restore log, then test recovery before the configured minimum interval.
- For atomicity, use a separate local QA campaign with two pending contacts and a nonzero interval. Issue concurrent authorized cron GET/POST requests before the first internal timer tick; HTTP 200 on all requests must still yield exactly one sent contact, one pending contact, one outbound and one campaign activity. A second immediate pass must not consume the other contact. Pause fixtures after testing; record any preexisting campaign changes.
- For published smoke, use an anonymous browser for Home/widget and /painel. Public widget lead/conversation IDs can be observed from actual /api/chat/messages responses with passive network inspection; this does not establish direct remote database coverage. Do not activate remote campaigns or call remote cron unless explicitly authorized.
